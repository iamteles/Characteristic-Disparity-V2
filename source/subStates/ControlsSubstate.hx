...
class ControlsSubstate extends MusicBeatSubState
{
    ...
    var songBpm:Float = 124; // lil' bit back bpm lmao
    ...
    public function new()
    {
        ...
        strumline = new Strumline(FlxG.width / 2, null, false, true, true, SaveData.skin());
        ...
        {
            var note = new Note();
            note.reloadNote(songBeat * i, i, "base", SaveData.skin());
            strumline.addNote(note);
        }

        metronome = new FlxSound().loadEmbedded(Paths.sound('hitsounds/cd'), false, false);
        metronome.play();
        metronome.stop();
        ...
        deadIcon.y = strumline.strumGroup.members[0].y - (deadIcon.height / 2) + 50;
        add(deadIcon);
        ...
        var offsetLabel = new FlxText(0, 0, 0, "SONG OFFSET");
        offsetLabel.setFormat(Main.gFont, 70, 0xFFFFFFFF, CENTER);
        offsetLabel.setBorderStyle(OUTLINE, 0xFF000000, 4);
        offsetLabel.scale.set(0.75,0.75);
        offsetLabel.updateHitbox();
        offsetLabel.text = "SONG OFFSET";
        offsetLabel.x = (offsetTxt.arrowL.x + offsetTxt.getWidth() / 2) - 160;
        ...
        */
        ...
        for(i in 0...optionShit.length)
        {
            ...
            item.setBorderStyle(OUTLINE, 0xFF000000, 4);
            item.scale.set(0.75,0.75);
            //offsetLabel.text = optionShit[i].toUpperCase();
            item.updateHitbox();
            item.screenCenter(X);
    ...
    function spawnBinds()
    {
        ...
        for(group in allGroups)
        {
            for(bind in group.members)
            {
                ...
                bind.y = strumline.strumGroup.members[0].y + 70 + 50;
                if(group == grpTxtTwo)
                    bind.y += 50; // 50
                ...
                if(bind.ID <= 3)
                    bind.x = strumline.strumGroup.members[bind.ID].x + 50;
                else
